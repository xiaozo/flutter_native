...
class AppNavigationToolbar extends StatelessWidget {
  /// Creates a widget that lays out its children in a manner suitable for a
  /// toolbar.
  const AppNavigationToolbar({
    Key key,
    this.leading,
    this.middle,
    this.trailing,
...
//      );
      ...
      switch (textDirection) {
        case TextDirection.rtl:
          leadingX = size.width - leadingWidth - navItemSpacing;
          middleMaxEndX = leadingX - middleSpacing;
          break;
        case TextDirection.ltr:
          leadingX = navItemSpacing;
    ...
    if (hasChild(_ToolbarSlot.trailing)) {
      ...
      switch (textDirection) {
        case TextDirection.rtl:
          trailingX = navItemSpacing;
          middleMinxStartX = trailingX + leadingWidth + middleSpacing;
          break;
        case TextDirection.ltr:
          trailingX = size.width - trailingSize.width - navItemSpacing;
          middleMaxEndX = trailingX - middleSpacing;
          break;
      }
      final double trailingY = (size.height - trailingSize.height) / 2.0;
      trailingWidth += navItemSpacing;
    ...
    if (hasChild(_ToolbarSlot.middle)) {
      ...
      if (centerMiddle) {
        ...
        if ((middleStart + middleSize.width > middleMaxEndX) ||
            (middleStart < middleMinxStartX)) {
          middleStart = middleMinxStartX;
        }
